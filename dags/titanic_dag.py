...
with DAG(
    ...
    schedule=None,
    catchup=False,
    description="Titanic survival prediction pipeline using Airflow",
) as dag:
    ...
    def evaluate_task(result):
        return evaluate_model(result)

    raw_data = load_data_task()
    processed_data = preprocess_task(raw_data)
    trained_result = train_task(processed_data)
    evaluate_task(trained_result)